...
- 监督损失对于训练的效果至关重要。由于存在以下的情况：
  ...
  2.数据中并不包含：(某一特定状态，所有可能执行的动作) 的完整状态动作对,
  许多 *状态动作对* 从未在专家样本集合中出现过。如果我们仅使用Q-learning的loss朝着下一个状态的最大Q值方向来更新Q网络，网络将倾向于朝着那些不准确的Q值中的最高方向更新,
  并且将在整个学习过程中通过Q函数传播这些误差，误差很容易累积，造成过估计问题。这里采用了 `监督大边际分类损失(supervised large margin classification loss) <https://arxiv.org/pdf/1606.01128.pdf>`_  来缓解这个问题，
  其计算公式为：
