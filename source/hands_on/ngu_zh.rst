...
通过学习一系列定向探索策略(directed exploratory policies)来解决困难的探索游戏。它的内在奖励分为2部分，第一部分是局内内在奖励(episodic intrinsic reward)，它是基于智能体最近一局经验的k近邻计算得到的，
自监督逆动力学模型用于训练最近邻查找的嵌入，将新颖性信号偏向代理可以控制的内容。
我们采用通用价值函数逼近器 (UVFA) 的框架，使用相同的神经网络同时学习许多定向探索策略，在探索和开发之间进行不同的权衡。
通过对不同程度的探索/利用使用相同的神经网络，从主要探索性政策产生有效的剥削性政策证明了转移。
所提出的方法可以与现代分布式 RL 代理一起运行，这些代理从在不同环境实例上并行运行的许多参与者收集大量经验。
我们的方法使基础代理在 Atari-57 套件中的所有硬探索中的性能翻倍，同时在其余游戏中保持非常高的分数，获得了 1344.0% 的人类标准化分数中位数。
值得注意的是，所提出的方法是第一个在 Pitfall 游戏中实现非零奖励（平均得分为 8,400）的算法！不使用演示或手工制作的功能。.
